...
class Volume extends EventEmitter {
    constructor(inConfig) {
        ...
        Object.defineProperty(this, 'isWritable', { get: this._isWritable });
    ...
    verify() {
        return new Promise(async (resolve, reject) => {
            ...
            let volumeUuidBuffer = Buffer.from(this.uuid.replace(/[^0-9a-f]/g, ''), 'hex');
            if (data.compare(volumeUuidBuffer, 0, 16, 21, 37) != 0)
    ...
    async createTemporaryFh(fileName) {
        ...
        let fileHandle = fsp.open(this.mountPoint + '/strubs/.tmp/' + fileName, 'wx');
        return fileHandle;
    }
    ...
    async destroyTemporaryFile(fileName) {
        let path = this.mountPoint + '/strubs/.tmp/' + fileName;
        await fsp.unlink(path);
    }

    async destroyCommittedFile(fileName) {
