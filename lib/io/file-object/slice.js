...
class Slice {
    ...
    async readChunk(chunkSize) {
        ...
        await this._inputFh.read(readBuf, 0, readLen);
    ...
    // TODO: enable async destroy if object isn't committed to deal with "failed to create slices", etc
    async destroy() {
        if (this._isPerformingIO)
            throw new Error('slice busy');
    ...
    throwChecksumError() {
        let err = new Error('checksum mismatch');
        err.code = 'ECHECKSUM';
        throw err;
    }
}
