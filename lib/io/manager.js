...
class IOManager {
    ...
    _queryOnlineDevices() {
        return new Promise(async (resolve, reject) => {
            ...
            for (let deviceIndex in result.blockdevices) {
                let blkDevice = result.blockdevices[deviceIndex];
                ...
                if (deviceNamesBySerial[blkDevice.serial]) {
                    log('skipping device ' + blkDevice.name + ' with same serial as device ' + deviceNamesBySerial[blkDevice.serial].name);
                    continue;
                }
    ...
    _cacheOnlineDevice(blkDevice) {
        return new Promise(async (resolve, reject) => {
            ...
            try {
                ...
                return reject(helpers.createErrorWithParams('cannot read sysfs path for device %s, model %s, serial %s:', [ blkDevice.name, blkDevice.model, blkDevice.serial ], err));
            }
...
_startDisks() {
    ...
    database._collections.disks.insertMany(disks);
}
*/