...
class FuseServer {
    ...
    async fuse_read(path, fd, buffer, length, position, cb) {
        ...
        // log('read lock acq', readId);

        object.setReadRange(position, endPosition);
        // log('read range set', readId);

        let bufferOffset = 0;
        let bytesRemaining = endPosition - position;

        object.on('data', data => {
            // log('read data', readId, `@${bufferOffset}`, `+${data.length}`);
            ...
            if (bytesRemaining == 0) {
                // log('read complete', readId, path, fd, `@${position}`, `+${length}`);
        ...
        object.on('error', err => {
            log('read error', /*readId,*/ path, fd, `@${position}`, `+${length}`, err);
            object.removeAllListeners();
            object.releaseIOLock();
            cb(Fuse.EREMOTEIO);
        });
